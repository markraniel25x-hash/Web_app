...
export default function Sidebar({ collapsed, activeTab, setActiveTab, user, logout }) {
  ...
  return (
    <aside className={`h-screen sticky top-0 left-0 bg-slate-900 border-r border-slate-800 flex flex-col transition-all duration-300 z-50 ${collapsed ? 'w-0 overflow-hidden' : 'w-[260px]'} select-none`}>
      {/* Brand logo container (Slate-navy base, vibrant electric blue details) */}
      <div className="p-5 border-b border-slate-800 flex items-center gap-3">
        <div className="w-9 h-9 bg-sky-500/10 rounded-lg flex items-center justify-center border border-sky-500/20 p-1">
          <img src={logo.src || logo} alt="ASA Philippines" className="object-contain" />
        </div>
        <span className="font-extrabold text-sm tracking-widest text-slate-100">BUDGET SYSTEM</span>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 flex flex-col gap-1.5">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`transition-colors ${isActive ? 'text-sky-400' : 'text-slate-500'}`}>{item.icon}</span>
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-950/20">
        <div className="flex items-center gap-3 p-2 bg-slate-800/40 rounded-xl border border-slate-800/80 shadow-sm">
          <div className="w-9 h-9 rounded-lg bg-sky-500 text-white font-extrabold text-xs flex items-center justify-center shadow-sm">
          ...
          <button 
            className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:bg-red-500/10 hover:text-red-400 transition-colors cursor-pointer" 
            onClick={logout} 
            title="Logout"
          >
            <LogOut size={16} />
